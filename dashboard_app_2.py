...
def order_sentiment_score_columns(matrix: pd.DataFrame) -> pd.DataFrame:
    desired = [1, 2, 3, 4, 5]
    existing = [col for col in desired if col in matrix.columns]
    remainder = [col for col in matrix.columns if col not in existing]
    return matrix.reindex(columns=existing + remainder, fill_value=0)
...
def build_dashboard_data(csv_path: Path = CSV_PATH) -> dict[str, Any]:
    ...
    call_type_vs_product_heatmap = pd.crosstab(
        known_products["product_focus_clean"],
        known_products["type of call"],
        dropna=False,
    )

    product_vs_sentiment_heatmap = order_sentiment_score_columns(
        pd.crosstab(
            known_products["sentiment_score"],
            known_products["product_focus_clean"],
            dropna=False,
        )
    )

    call_type_vs_sentiment_heatmap = order_sentiment_score_columns(
        pd.crosstab(
            df["type of call"],
            df["sentiment_score"],
            dropna=False,
    ...
    return {
        ...
        "call_type_vs_product_heatmap": call_type_vs_product_heatmap,
        "product_vs_sentiment_heatmap": product_vs_sentiment_heatmap,
        "call_type_vs_sentiment_heatmap": call_type_vs_sentiment_heatmap,
    }
...
def heatmap_html(df: pd.DataFrame, title: str, x_label: str, y_label: str) -> str:
    if df.empty:
        return f"""
        <div class="card">
            <h3>{escape(title)}</h3>
    ...
    headers = ''.join(f'<th>{escape(str(col))}</th>' for col in matrix.columns)
    rows = []
    for idx, row in matrix.iterrows():
        cells = []
        for col in matrix.columns:
            value = int(row[col])
            alpha = 0.08 + (0.82 * (value / max_value)) if value > 0 else 0.04
            text_color = '#e5e7eb' if value < (max_value * 0.55) else '#08111f'
            style = (
                f'background: rgba(56, 189, 248, {alpha:.3f}); '
                f'color: {text_color}; '
                'font-weight: 600; text-align: center; min-width: 64px;'
            )
            cells.append(f'<td class="heatmap-cell" style="{style}" title="{escape(str(idx))} / {escape(str(col))}: {value}">{value}</td>')
        rows.append(f"<tr><th class='heatmap-row-label'>{escape(str(idx))}</th>{''.join(cells)}</tr>")

    return f"""
    <div class="card">
        <h3>{escape(title)}</h3>
        <div class="section-note">X-axis: {escape(x_label)} · Y-axis: {escape(y_label)}</div>
        <div class="table-wrap heatmap-wrap">
            <table class="heatmap-table">
                <thead>
                    <tr>
                        <th>{escape(y_label)} / {escape(x_label)}</th>
                        {headers}
                    </tr>
                </thead>
                <tbody>
...
def dashboard() -> str:
    ...
    heatmaps_tab = f"""
    <div id="tab-heatmaps" class="tab-panel">
        <div class="grid section">
            {heatmap_html(data["call_type_vs_product_heatmap"], "Heatmap: Call Type vs Product", "Product", "Call Type")}
            {heatmap_html(data["product_vs_sentiment_heatmap"], "Heatmap: Product vs Sentiment", "Sentiment Score", "Product")}
            {heatmap_html(data["call_type_vs_sentiment_heatmap"], "Heatmap: Call Type vs Sentiment", "Sentiment Score", "Call Type")}
        </div>
    </div>
    """
    ...
    <body>
        <div class="container">
            <h1>Call Analysis Dashboard</h1>
            <p class="sub">
                Please select the tabs below to view each session. 
            ...
            <div class="tabs">
                ...
                <button class="tab-button" onclick="showTab('heatmaps', this)">Heat map</button>
            </div>
...
def api_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "call_type_vs_product_heatmap": data["call_type_vs_product_heatmap"].reset_index().to_dict(orient="records"),
        "product_vs_sentiment_heatmap": data["product_vs_sentiment_heatmap"].reset_index().to_dict(orient="records"),
        "call_type_vs_sentiment_heatmap": data["call_type_vs_sentiment_heatmap"].reset_index().to_dict(orient="records"),
    }
